...
def _create_neo4j_graph(*, url, username, password):
    """
    Try to create a Neo4jGraph from langchain_community if available.
    Return None if the dependency is missing instead of crashing.
    """
    try:
        graphs_module = import_module("langchain_community.graphs")
    except ModuleNotFoundError as exc:
        logging.warning(
            "langchain-community / Neo4jGraph not available: %s. "
            "Graph RAG will be disabled.",
            exc,
        )
        return None
...
def _create_neo4j_chain(graph, model_name):
    """
    Try to create a Neo4jGraphQAChain from langchain if available.
    Return None if dependencies are missing instead of crashing.
    """
    if graph is None:
        return None
    try:
        ...
        logging.warning(
            "langchain / langchain-openai not available: %s. "
            "Graph RAG will fall back to simple in-memory mode.",
            exc,
        )
        return None
...
class GraphRAG:
    ...
    async def index_document(self, path):
        """
        Index a resume:
        - Read the file text
        ...
        # If graph mode is enabled, parse resume into Neo4j as well.
        if self._use_graph and self.graph is not None:
            candidate_id = str(path_obj)  # or generate your own stable ID
            await asyncio.to_thread(
                self._write_resume_to_graph, candidate_id, resume_struct
            )
...
class ChatAgent:
    ...
    async def _call_planner(
        ...
        required_slots = {
            ...
            "total_experience": "Total years of relevant work experience.",
        ...
        system_prompt = (
            ...
            "  4) project_metric\n"
            "  5) project_bottleneck\n"
            "  6) project_solution\n"
            "  7) team_challenge\n"
            "  8) adaptability_example\n"
            "  9) leadership_example\n"
            "  10) notice_period\n"
            "  11) visa_status\n\n"
            "You are also given:\n"
            ...
            "3. When possible, infer total_experience and project-related slots directly from "
            "   resume_struct (experiences list) or cv_context, instead of asking the user to repeat "
            "   obvious CV facts.\n"
